...
fn version_parts() -> (String, String, String) {
    ...
    let manifest = format!("{major}.{minor}.{patch}.0");
    (package_version, tuple, manifest)
}

fn manifest_with_version(template: &str, version: &str) -> Result<String, String> {
    let identity = template
        .find("<assemblyIdentity")
        .ok_or("app.manifest is missing assemblyIdentity")?;
    let version_marker = "version=\"";
    let relative = template[identity..]
        .find(version_marker)
        .ok_or("app.manifest assemblyIdentity is missing version")?;
    let value_start = identity + relative + version_marker.len();
    let value_end = value_start
        + template[value_start..]
            .find('"')
            .ok_or("app.manifest assemblyIdentity version is not terminated")?;

    let mut rendered = String::with_capacity(template.len() + version.len());
    rendered.push_str(&template[..value_start]);
    rendered.push_str(version);
    rendered.push_str(&template[value_end..]);
    Ok(rendered)
}

fn generated_rc(manifest: &Path, icon: &Path) -> String {
    let (package_version, tuple, _) = version_parts();
    format!(
...
fn main() {
    ...
    const MANIFEST_TEMPLATE: &str = "app.manifest";

    println!("cargo:rerun-if-changed={MANIFEST_TEMPLATE}");
    ...
    let manifest_path = out_dir.join("RepartoCopier.manifest");
    ...
    fs::write(&manifest_path, manifest).expect("write generated app.manifest to OUT_DIR");
...
mod tests {
    ...
    #[test]
    fn only_primary_assembly_identity_version_is_replaced() {
        let template = r#"<assembly><assemblyIdentity version="1.3.0.0"/><dependency><assemblyIdentity version="6.0.0.0"/></dependency></assembly>"#;
        let rendered = manifest_with_version(template, "1.4.1.0").unwrap();
        assert!(rendered.contains("<assemblyIdentity version=\"1.4.1.0\"/>"));
        assert!(rendered.contains("<assemblyIdentity version=\"6.0.0.0\"/>"));
    }
}
