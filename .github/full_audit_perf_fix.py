...
start = t.find(needle)
if start < 0:
    raise RuntimeError('skip_same test not found')
end = t.find('\n    #[test]', start + len(needle))
if end < 0:
    raise RuntimeError('skip_same test end not found')
block = t[start:end]
block = replace_one(
    block,
...
print('dead verification phase and stale test expectation removed')
