...
impl DuplicatorApp {
    pub fn new() -> Self {
        ...
        Self {
            ...
            selected,
            hide_system: true,
            status: "Selecciona origen y destinos. Iniciar pisa los destinos.".into(),
            job: None,
            workers: Vec::new(),
        }
    }
    ...
    fn refresh(&mut self) {
        ...
        match enumerate_disks() {
            Ok(d) => {
                self.disks = d;
                self.selected.resize(self.disks.len(), false);
    ...
    fn problems(&self) -> Vec<String> {
        ...
        let Some(src) = self.source_disk() else {
            p.push("No hay origen.".into());
            return p;
        };
        if src.is_system {
            p.push("El origen es el disco de sistema.".into());
        }
        let dests = self.dest_disks();
        if dests.is_empty() {
            p.push("No hay destinos válidos.".into());
        }
        for d in &dests {
            if d.size_bytes < src.size_bytes {
                p.push(format!(
                    "PD{} es más pequeño que el origen ({} GB < {} GB).",
                    d.index,
                    d.size_gb(),
                    src.size_gb()
                ));
    ...
    fn start(&mut self) {
        ...
        let dests = self.dest_disks();
        self.status = format!(
            "Copiando PD{} → {} destino(s). Cierra Explorer en esos volúmenes.",
            src.index,
            dests.len()
        );
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::right("side").min_width(320.0).show(ctx, |ui| {
            ...
            if dests.is_empty() {
                ...
                for d in &dests {
                    ui.label(format!("PD{}  {}  {} GB", d.index, d.model, d.size_gb()));
                }
            }
            ui.add_space(8.0);
            ...
            if probs.is_empty() {
                ...
                for p in probs {
                    ui.colored_label(Color32::from_rgb(220, 160, 80), format!("• {p}"));
                }
            }
            ui.add_space(12.0);
            ui.horizontal(|ui| {
                let can = self.problems().is_empty()
                    && !self.job.as_ref().is_some_and(|j| j.running.load(std::sync::atomic::Ordering::Relaxed));
                ui.add_enabled_ui(can, |ui| {
                    if ui
                        .add(egui::Button::new(RichText::new("Iniciar").strong()).fill(ACCENT))
                        .clicked()
                    {
                        self.start();
                    }
