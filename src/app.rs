...
impl CopierApp {
    pub fn new() -> Self {
        Self {
            source: String::new(), dests: Vec::new(), verify: false, skip_same: true, keep_going: true,
            status: "Copiador de archivos desde una carpeta de origen a múltiples destinos".into(),
            job: None, workers: Vec::new(), startup_rx: None, show_credits: false, error_flash_until: None,
            use_light_theme: detect_system_theme(), last_theme_check: Instant::now(), applied_theme: None,
            fonts_initialized: false, path_errors: Vec::new(), last_path_check: Instant::now(), paths_key: u64::MAX,
        }
    }
    fn starting(&self) -> bool { self.startup_rx.is_some() }
    fn running_job(&self) -> bool { self.job.as_ref().is_some_and(|j| j.running.load(Ordering::Relaxed)) }
    fn pick_dir() -> Option<String> { rfd::FileDialog::new().pick_folder().map(|p| p.to_string_lossy().into_owned()) }
    fn validate_paths(&self) -> Vec<String> {
        ...
        for (i, dest) in self.dests.iter().enumerate() {
            ...
            if !path.exists() { errors.push(format!("El destino {} no existe o no está disponible.", i + 1)); }
            else if !path.is_dir() { errors.push(format!("El destino {} no es una carpeta.", i + 1)); }
        }
        errors
    }
    fn paths_key(&self) -> u64 {
        let mut hasher = DefaultHasher::new();
        self.source.hash(&mut hasher); self.dests.hash(&mut hasher); hasher.finish()
    }
    fn flash_error(&mut self, message: String) { self.status = message; self.error_flash_until = Some(Instant::now() + ERROR_FLASH); }
    fn start(&mut self) {
        if self.source.trim().is_empty() { self.flash_error("Selecciona una carpeta de origen.".into()); return; }
        if self.dests.is_empty() { self.flash_error("Agrega al menos un destino.".into()); return; }
        if let Some(first) = self.validate_paths().into_iter().next() { self.flash_error(first); return; }
        ...
        self.job = None; self.workers.clear(); self.error_flash_until = None;
        self.status = "Analizando origen y destinos…".into(); self.startup_rx = Some(rx);
        thread::spawn(move || { let _ = tx.send(start_job(src, dests, opts)); });
    }
    fn poll_startup(&mut self) {
        ...
        match outcome {
            Ok(Ok((state, handles))) => {
                ...
                self.status = format!("Comprobación correcta · {} · {} · {}", count_label(n_files, "archivo", "archivos"), format_bytes(state.bytes_total.load(Ordering::Relaxed)), count_label(n_dests, "destino", "destinos"));
                self.job = Some(state); self.workers = handles;
            }
...
impl eframe::App for CopierApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        if !self.fonts_initialized { setup_fonts(ctx); self.fonts_initialized = true; }
        self.poll_startup();
        if self.last_theme_check.elapsed() >= THEME_CHECK_INTERVAL { self.use_light_theme = detect_system_theme(); self.last_theme_check = Instant::now(); }
        if self.applied_theme != Some(self.use_light_theme) { apply_theme(ctx, self.use_light_theme); self.applied_theme = Some(self.use_light_theme); }
        let snaps = self.job.as_ref().map(|job| job.snapshot()).unwrap_or_default();
        let all_terminal = !snaps.is_empty() && snaps.iter().all(|d| matches!(d.phase, DestPhase::Done | DestPhase::Failed | DestPhase::Cancelled));
        ...
        if let Some(until) = self.error_flash_until {
            if now >= until { self.error_flash_until = None; } else { ctx.request_repaint_after(until.saturating_duration_since(now)); }
        }
        if starting { ctx.request_repaint_after(STARTING_REPAINT); }
        else if running {
            let paused = self.job.as_ref().is_some_and(|j| j.pause.load(Ordering::Relaxed));
            ctx.request_repaint_after(if paused { PAUSED_REPAINT } else { RUNNING_REPAINT });
        } else { ctx.request_repaint_after(THEME_CHECK_INTERVAL); }
        let key = self.paths_key();
        if key != self.paths_key || self.last_path_check.elapsed() >= PATH_CHECK_INTERVAL {
            self.paths_key = key; self.last_path_check = Instant::now(); self.path_errors = if busy { Vec::new() } else { self.validate_paths() };
        }
        ...
        egui::TopBottomPanel::top("header").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.heading(RichText::new("RepartoCopier").strong()); ui.weak("Copiador múltiple de archivos");
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| { if starting { ui.weak("Validando..."); } });
            });
            ui.add_space(SPACING_XS);
        });
        egui::TopBottomPanel::bottom("footer").show(ctx, |ui| {
            ui.horizontal(|ui| {
                if error_flash_active { ui.colored_label(Theme::error(self.use_light_theme), &self.status); } else { ui.weak(&self.status); }
                if starting { ui.separator(); ui.weak("Analizando archivos..."); } else if running { ui.separator(); ui.weak("Copiando en paralelo"); }
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| { if ui.small_button("Créditos").clicked() { self.show_credits = true; } });
            });
        });
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.add_enabled_ui(!busy, |ui| {
                    ui.add(egui::TextEdit::singleline(&mut self.source).desired_width(ui.available_width() - 90.0).hint_text("Carpeta origen"));
                    if ui.button("Examinar").clicked() { if let Some(p) = Self::pick_dir() { self.source = p; } }
                });
            });
            ui.add_space(SPACING_XS);
            ui.horizontal(|ui| {
                ...
                ui.add_enabled_ui(!busy, |ui| { if ui.button("+ Agregar").clicked() { if let Some(p) = Self::pick_dir() { if !self.dests.contains(&p) { self.dests.push(p); } } } });
            });
            egui::ScrollArea::vertical().id_salt("destinations").max_height(150.0).show(ui, |ui| {
                let mut remove = None;
                for (i, d) in self.dests.iter().enumerate() {
                    ui.horizontal(|ui| {
                        ui.weak(format!("{:02}", i + 1)); ui.label(compact_path(d, 66)).on_hover_text(d);
                        if let Some(dp) = snaps.get(i) { let (label, color) = phase_label(dp.phase, dp.files_err, self.use_light_theme); ui.colored_label(color, format!("[{label}]")); }
                        if !busy && ui.small_button("×").clicked() { remove = Some(i); }
                    });
                }
                if let Some(i) = remove { self.dests.remove(i); }
            });
            if path_error_count > 0 { ui.colored_label(Theme::warning(self.use_light_theme), format!("⚠ {} problema(s) de ruta detectado(s)", path_error_count)).on_hover_text(&path_error_hover); }
            ui.separator();
            ui.horizontal(|ui| {
                ui.add_enabled_ui(!busy, |ui| { ui.checkbox(&mut self.skip_same, "Omitir iguales"); ui.checkbox(&mut self.keep_going, "Continuar con errores"); ui.checkbox(&mut self.verify, "Verificar BLAKE3"); });
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if running {
                        if let Some(job) = &self.job {
                            let paused = job.pause.load(Ordering::Relaxed);
                            if ui.button(if paused { "Continuar" } else { "Pausar" }).clicked() { job.pause.store(!paused, Ordering::Relaxed); }
                            if ui.button("Cancelar").clicked() { job.cancel.store(true, Ordering::Relaxed); job.pause.store(false, Ordering::Relaxed); }
                        }
                    } else if !starting {
            ...
            if starting {
                ...
                ui.centered_and_justified(|ui| { ui.label(RichText::new("Analizando origen y destinos…").size(20.0).strong()); });
                ...
                if !snaps.is_empty() {
                    ...
                    for dp in &snaps {
                        ...
                        if remaining == 0 || matches!(dp.phase, DestPhase::Done | DestPhase::Failed | DestPhase::Cancelled) { continue; }
                        if dp.bps <= 0.0 { eta_known = false; break; }
                    ...
                    ui.add(egui::ProgressBar::new(avg_progress as f32).desired_height(20.0).fill(Theme::progress_bar(self.use_light_theme)).show_percentage());
                    ui.horizontal(|ui| {
                        ui.heading(format_bps(total_bps));
                        ui.weak(format!("{} · Tiempo restante {}", format_bytes(job.bytes_total.load(Ordering::Relaxed)), eta_text));
                    });
                    ui.add_space(SPACING_SM);
                    egui::ScrollArea::vertical().id_salt("dest-progress").max_height(300.0).show(ui, |ui| {
                        egui::Grid::new("dest-grid").striped(true).num_columns(5).spacing([16.0, 8.0]).show(ui, |ui| {
                            ui.strong("Destino"); ui.strong("Velocidad"); ui.strong("Progreso"); ui.strong("Cola"); ui.strong("Estado"); ui.end_row();
                            for dp in &snaps {
                                let frac = if dp.total == 0 { if dp.phase == DestPhase::Done { 1.0 } else { 0.0 } } else { (dp.written as f32 / dp.total as f32).clamp(0.0, 1.0) };
                    ...
                    ui.add_space(SPACING_SM);
                    ui.horizontal(|ui| {
                        ui.weak(format!("Memoria {}/{}", job.buffers_in_flight.load(Ordering::Relaxed), job.max_buffers));
                        ui.separator(); ui.weak(count_label(snaps.len() as u64, "destino", "destinos")); ui.separator();
                        let retries: u64 = snaps.iter().map(|d| d.retries).sum(); ui.weak(count_label(retries, "reintento", "reintentos"));
                    });
                }
            } else {
                ui.add_space(SPACING_MD);
                ui.centered_and_justified(|ui| { ui.label(RichText::new("Listo para copiar").size(22.0).strong()); });
            }
        });
        if self.show_credits {
            let mut open = true;
            egui::Window::new("Créditos").collapsible(false).resizable(false).default_width(400.0).open(&mut open).show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.heading(RichText::new("RepartoCopier").strong()); ui.label(format!("Versión {}", env!("CARGO_PKG_VERSION")));
                    ui.add_space(SPACING_MD); ui.label(RichText::new("Desarrollado por").small()); ui.heading(RichText::new("ReinierTutoriales").strong().size(16.0)); ui.label(RichText::new("© 2026").small());
                    ui.add_space(SPACING_MD); ui.separator(); ui.add_space(SPACING_SM); ui.label("Tecnologías");
                    ui.horizontal_wrapped(|ui| { ui.label("Rust"); ui.separator(); ui.label("egui / eframe"); ui.separator(); ui.label("BLAKE3"); });
                    ui.add_space(SPACING_SM); ui.label(RichText::new("Copiador de archivos desde una carpeta a múltiples discos").small());
                    ui.add_space(SPACING_MD); ui.separator(); ui.add_space(SPACING_SM); ui.label("Web principal");
                    ui.hyperlink_to("github.com/ReinierTutoriales/disk-duplicator", "https://github.com/ReinierTutoriales/disk-duplicator");
                    ui.add_space(SPACING_SM); ui.label(RichText::new("Licencia MIT").small());
                });
            });
